...
exports.create = (req, res) => {
    ...
    convenio
        ...
        .then((data) => {
            res.send({
                message: "Se creo el convenio correctamente",
                data: data,
            });
        })
        .catch((err) => {
...
exports.findOne = (req, res) => {
    ...
    Convenio.findById(id)
        ...
        .then((data) => {
            if (!data)
                res.status(404).send({
                    message: "No se encontro una convenio con ese Id.",
                });
            else res.send(data);
        })
        .catch((err) => {
...
exports.update = (req, res) => {
    ...
    Convenio.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
        .then((data) => {
            if (!data)
                res.status(404).send({
                    message: "No se encontro un Convenio con ese Id.",
                });
...
exports.agregarCategoria = (req, res) => {
    ...
    Convenio.findById(id)
        ...
        .then((data) => {
            cat_agregada = data;
            return Convenio.findByIdAndUpdate(
                id,
                {
                    $push: {
                        categorias: data.id,
        ...
        .then((data) => {
            res.send({
                message: "Se agrego la categoria al convenio correctamente",
                data: cat_agregada,
            });
        })
        .catch((err) => {
...
exports.quitarCategoria = (req, res) => {
    ...
    Convenio.findByIdAndUpdate(
        ...
        .then((data) => {
            CategoriaConv.findByIdAndRemove(idCat).then((data) => {
                if (!data) {
                    res.status(404).send({
...
exports.agregarSubCategoria = (req, res) => {
    const id = req.params.id;
    const idCat = req.body.idCat;
    if (!idCat) res.status(400).send({ message: "Falta el campo idCat" });
    ...
    Convenio.findById(id)
        ...
        .then((data) => {
            return CategoriaConv.findByIdAndUpdate(
                idCat,
                {
                    $push: {
        ...
        .then((data) => {
            res.send({
                message: "Se agrego la subcategoria al convenio correctamente",
                data: data,
            });
        })
        .catch((err) => {
...
exports.updateSubCategoria = (req, res) => {
    ...
    SubCategoriaConv.findByIdAndUpdate(idCat, req.body, {
        useFindAndModify: false,
    })
        .then((data) => {
            if (!data)
                res.status(404).send({
                    message: "No se encontro una sub categoria con ese Id.",
                });
            else
                res.send({ message: "Sub categoria Actualizada", id: data.id });
        })
        .catch((err) => {
            res.status(500).send({ message: err.message });
        });
...
exports.quitarSubCategoria = (req, res) => {
    ...
    CategoriaConv.findByIdAndUpdate(
        idCat,
        {
            $pull: {
        ...
        .then((data) => {
            SubCategoriaConv.findByIdAndRemove(idSubCat).then((data) => {
                if (!data) {
                    res.status(404).send({
...
exports.agregarSumaDescuento = (req, res) => {
    ...
    Convenio.findById(id)
        ...
        .then((data) => {
            res.send({
                message:
                    "Se agrego la suma o descuento al convenio correctamente",
                data: sumadesc_creada,
            });
        })
        .catch((err) => {
            res.status(500).send({ message: err.message });
...
exports.quitarSumaDescuento = (req, res) => {
    ...
    Convenio.findByIdAndUpdate(
        ...
        .then((data) => {
            SumasDescuentos.findByIdAndRemove(idSum).then((data) => {
                if (!data) {
                    res.status(404).send({
                        message:
                            "No se encontro La Suma o Descuento con ese id.",
                    });
                } else {
                    res.send({
